...
Open the Lab 4 project in the [course space on Posit Cloud](https://posit.cloud/spaces/787651) to get started.
:::

## Overview

Use a simulation-based workflow (bootstrap CI + hypothesis test) to analyze a governance reform example. Fill in each `???` with the correct code. 
...
### Confidence level comparison (10 pts)

Change the confidence level in `get_ci()` to `0.90` and re-run the visualization.
...
boot_df |>
  visualize() +
  shade_ci(ci_90, color = "red", fill = NULL) +
  labs(
    title = "Bootstrap Distribution with 90% CI",
    x = "Sample proportion asked for unofficial payment",
    y = "Count"
  ) +
  theme_minimal()
...
**Question:** Is the 90% CI wider or narrower than the 95% CI? Explain in 1-2 sentences why that makes sense.
...
Visualize the null distribution and shade the p-value region.
...
### Calculate the p-value (10 pts)

Use `get_p_value()` to calculate the p-value.
...
null_dist |>
  visualize() +
  shade_p_value(obs_stat = 0.12, direction = "less")
```
...
```
null_dist |>
  get_p_value(obs_stat = 0.12, direction = "less")
```
