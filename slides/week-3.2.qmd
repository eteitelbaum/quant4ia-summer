...
- Change the confidence level with `get_ci()` to `.90`. What happens to the width of the confidence interval?
- Visualize the new confidence interval
- How would you interpret this CI?
- Now try changing the confidence level to `.99`. What happens to the width of the confidence interval and how would you interpret it?
...
- Precision refers to variability: procedures with less sampling variability will be more precise
    - all else equal, a greater **sample size** (`n`) will increase precision and produce narrower CIs
- Increasing the number of bootstrap **reps** does *not* narrow the CI — it only makes the bootstrap distribution smoother and the CI estimate more stable
- To get a genuinely narrower CI, you need more data, not more resamples
