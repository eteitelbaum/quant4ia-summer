...
Before fitting a model, we can quantify the strength of the relationship:

```{r}
#| label: correlation
#| echo: true

r_cor <- round(cor(modelData$log_wealth, modelData$lib_dem, use = "complete.obs"), 2)
r_cor
```
...
::: {.incremental}
- **r ≈ `r r_cor`**: moderate positive relationship
- r = +1 (perfect positive), 0 (none), −1 (perfect negative)
...
fit_aug  <- augment(mod_fit) |>
  mutate(res_cat = ifelse(.resid > 0, TRUE, FALSE))

a <- round(coef(mod_fit)[1], 3)
...
ggplot(modelData, aes(x = log_wealth, y = lib_dem)) +
  ...
  geom_smooth(method = "lm", color = "#E48957", se = FALSE) +
  labs(x = "GDP per capita", y = "Liberal Democracy Index") +
  geom_segment(aes(x = 2, xend= 3, y=a+b*2, yend=a+b*2), colour="darkblue", linewidth=1.5, arrow = arrow(length = unit(0.5, "cm"))) +
  theme_bw()
...
ggplot(modelData, aes(x = log_wealth, y = lib_dem)) +
  ...
  geom_smooth(method = "lm", color = "#E48957", se = FALSE) +
  labs(x = "GDP per capita", y = "Liberal Democracy Index") +
  geom_segment(aes(x = 2, xend= 3, y=a+b*2, yend=a+b*2), colour="darkblue", linewidth=1.5) +
  geom_segment(aes(x = 3, xend= 3, y=a+b*2, yend=a+b*3), colour="darkblue", linewidth=1.5, arrow = arrow(length = unit(0.5, "cm"))) +
  geom_text(x=3.2, y=a+b*2.5, label=b, color="darkblue", size=4) +
  theme_bw()
...
$$\widehat{Democracy}_{i} = `r a` + `r b` \times {loggdppc}_{i}$$

## R-Squared

<br>

How well does the model fit? **R²** tells us the proportion of variation in Y explained by X:

```{r}
#| label: r-squared
#| echo: true

r2 <- round(summary(model1)$r.squared, 2)
r2
```
...
An R² of ≈ `r r2` means log GDP per capita explains **`r round(r2 * 100)`%** of the variation in democracy scores. The remaining `r round((1 - r2) * 100)`% is explained by other factors.
...
- See starter code in the classwork folder on Dropbox
