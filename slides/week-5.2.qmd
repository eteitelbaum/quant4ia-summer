...
```{r}
#| echo: false
a2 <- round(coef(model2)[1], 3)
b1 <- round(coef(model2)[2], 3)
b2 <- round(coef(model2)[3], 3)
```
...
$$\hat{Y_i} = `r a2` + `r b1`*Polarization + `r b2`*GDPpc$$
...
```{r}
#| echo: false
a3 <- round(coef(model3)[1], 3)
b1_3 <- round(coef(model3)[2], 3)
b2_3 <- round(coef(model3)[3], 3)
b3_3 <- round(coef(model3)[4], 5)
```
...
$$\hat{Y_i} = `r a3` + `r b1_3`*Polarization + `r b2_3`*GDPpc + `r b3_3`*OilRents$$
<br>
...
```{r}
#| echo: false
jr_a <- round(coef(jr_dem_model)[1], 3)
jr_b <- round(coef(jr_dem_model)[2], 3)
```
...
$$\widehat{Democracy_{i}} = `r jr_a` + `r jr_b`*JudicialReview(yes)$$

- **Slope:** countries with judicial review are expected, on average, to be `r jr_b` units more democratic on the liberal democracy index
...
- Average democracy score of countries with judicial review is `r jr_a` + `r jr_b` = `r jr_a + jr_b`
...
- Filter data to include only the year 2006 (or run the code to use modelData)
