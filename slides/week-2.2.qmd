...
- Now let's focus on categorical data
- These variables place observations into groups
- Today we will use V-Dem's `v2x_regime` variable
- It classifies each country into a regime type
...
```{r}
#| label: regime-glimpse
#| echo: true

glimpse(vdem2022_regime)
```
...
- You only need to specify the x aesthetic
- `geom_col()` is for pre-computed values like the regional means we graphed earlier

## Your Turn!

- Change the year and re-run the setup
- Visualize the distribution of regime types with `geom_bar()`
- Update the plot title to match your year
- What is different about your year relative to 2022?
...
:::{.incremental}
- We used `geom_bar()` with two categorical variables
- `regime` is an ordinal categorical variable
- `region` is a nominal categorical variable
- The plot shows counts, not proportions
:::
...
Some regions have more countries than others.

Why does that create an issue for comparing raw bar heights here?

```{r}
#| label: regime-region3
...
- We use `position = "fill"` to normalize each bar to 100%
- That makes it easier to compare regions with different numbers of countries
- This is usually the better choice when the group sizes are unequal

## Your Turn!

- Change the year to a different one
- Repeat the region comparison with `fill = regime`
- Use `position = "fill"` to compare proportions
- Bonus: try `position = "dodge"` and compare what it emphasizes
...
You work at a think tank and your boss wants you to...
