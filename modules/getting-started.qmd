...
## Before the First Class {#sec-before-class}

Please complete the setup steps below **before the first class session**. Both setups are required — Posit Cloud for in-class work, and a local installation for labs and homework. If you run into any issues, email me at [ejt@gwu.edu](mailto:ejt@gwu.edu) so we can sort it out before class starts.

### Option 1: Posit Cloud (Required for In-Class Work)

We will use [Posit Cloud](https://posit.cloud) for all in-class activities. It runs R and RStudio directly in your browser — no installation needed, and everyone works in the same environment.

**Step 1:** Go to [posit.cloud](https://posit.cloud) and sign up for a free account if you don't already have one.

**Step 2:** Subscribe to the **Cloud Student** plan ($5/month). You can cancel at the end of the semester. To subscribe:

1. Click your name in the upper right corner
2. Select **Subscription**
3. Choose the **Cloud Student** plan and enter your payment information

**Step 3:** Click the link below to join our course Space on Posit Cloud. You will need to be logged in and subscribed before the link will work.

::: {.callout-important}
**Course Space Link:** [Click here to join the course Space on Posit Cloud](https://posit.cloud/spaces/787651/join?access_code=5XMPcLPUUmYp9h7YBC-IXpFnfdT50Yuy8IoQW1xh)
:::

Once you click the link and join, you will see the course workspace with all project files pre-loaded. This is where we will work during class sessions.

**Step 4:** Open any project in the course Space and run the following in the R console to confirm everything is working:
...
If you see `[1] "Setup complete!"` with no errors, you are good to go.

---

### Option 2: Local Installation (Required for Labs and Homework)

For labs and homework assignments you will work locally on your own computer. Posit Cloud can be used as a fallback if needed, but local installation is strongly preferred.

**Step 1:** Download and install R from [CRAN](https://cran.r-project.org). Choose the version for your operating system (Windows, Mac, or Linux).

**Step 2:** Download and install [RStudio Desktop](https://posit.co/download/rstudio-desktop/) (the free version). RStudio is the IDE we will use to write and run R code throughout the course.

**Step 3:** Quarto comes bundled with recent versions of RStudio, but if you need to install it separately, download it from [quarto.org](https://quarto.org/docs/get-started/).

**Step 4:** Open RStudio and run the following in the console to install the packages we will use throughout the course:
...
`vdemdata` is not on CRAN and must be installed from GitHub. If the command above fails for `vdemdata`, run:
...
**Step 5:** Create a new Quarto document (File → New File → Quarto Document), add a code chunk, and run:
...
If you see a scatter plot, your local setup is complete.
