...
Install the following packages and familiarize yourself with the basic purpose and documentation of each:

- [peacesciencer](http://svmiller.com/peacesciencer/) — conflict data tools
- [broom](https://broom.tidymodels.org/) — tidying model output
- [modelsummary](https://modelsummary.com/) — publication-ready tables
- [marginaleffects](https://marginaleffects.com/) — predicted probabilities and marginal effects

```r
install.packages(c("peacesciencer", "broom", "modelsummary", "marginaleffects"))
```
:::
...
Coefficient plots show effect sizes and uncertainty, but they work on the **log-odds scale**, which is difficult to interpret in practical terms. A more intuitive complement is a **predicted probability plot**, which shows how the probability of the outcome changes across a range of values for a key predictor.

The `plot_predictions()` function from `marginaleffects` makes this straightforward. Let's show how predicted conflict probability varies with GDP per capita:

```{r}
library(marginaleffects)

plot_predictions(conflict_model,
                 condition = "wbgdppc2011est") +
  labs(
    title = "Predicted Probability of Conflict Onset by GDP per Capita",
...
This translates the abstract coefficient into a concrete story: countries with very low GDP per capita face substantially higher predicted conflict probabilities than wealthier countries. The confidence interval (shaded area) shows our uncertainty around these predictions.

::: {.callout-warning icon=false}
## Your Turn

Recreate the regression table or coefficient plot, but this time try an alternative specification:

1. Replace `v2x_polyarchy` with `ethfrac` (ethnic fractionalization) or `relfrac` (religious fractionalization) in one of the models.
2. Re-run `modelsummary()` to compare the new specification side by side with the others.
3. Use `plot_predictions()` to visualize how conflict probability varies with your chosen variable.

What do you find? Do ethnic or religious fractionalization predict conflict onset after controlling for GDP and population?
:::
